    ...
    match state:
        States.MINING:
            ...
            if not _asteroid:
                ...
                var available_space: float = stats.get_max_cargo() - stats.cargo
                var requested_amount: float = min(available_space, stats.get_mining_rate() * delta)
                
                var mined: float = _asteroid.mine_amount(requested_amount)
